...
def create_advection_animation(output_path='./output', n_frames=60, fps=8):
    ...
    for step in range(n_frames):
        ...
        R_occ = compute_occlusion_risk(shadow_mask, X, Y, truck)
               ...
               # Smooth component fields for softer transitions
        R_vehicles = gaussian_filter(R_vehicles, sigma=0.8)
        R_merge = gaussian_filter(R_merge, sigma=1.2)
        R_occ = gaussian_filter(R_occ, sigma=0.8)

        # Blend merge risk with a continuous spatial envelope (pre/post merge)
        pre = cfg.merge_x_start - 15
        post = cfg.merge_x_end + 25
        ramp = np.clip((X - pre) / (post - pre), 0, 1)
        w_on = (3 * ramp**2 - 2 * ramp**3)
        # Slower decay past merge end to avoid abrupt drop
        tail = np.where(X <= cfg.merge_x_end, 1.0, np.exp(-((X - cfg.merge_x_end) / 100.0)**2))
        w_merge = w_on * tail
